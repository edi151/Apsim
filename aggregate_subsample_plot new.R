#------ Set the working directory where the Out files are located -------------

setwd("C:\\Users\\DunwoodyE\\Documents\\Apsim73\\Regions\\M_W region\\7.3ComparisonSample\\Test Out files")

# The next command creates an object called filesList
filesList <- list.files()

# Next command creates a data frame
...
#print(filesList[4])
    ...
    {
#file To Read in = ""C:\Users\DunwoodyE\Documents\Apsim73\Regions\M_W region\7.3ComparisonSample\Test Out files\mariBfBfBf_1970_14735_ADJ_1902_BU_RC6_delta_w4 daily.out"" 
      #Not clear why this has a specific file in it but it is only a comment line
  	all_content = readLines(filesList[j]) # this is all of the data in the file
  	all_data_except_second_row = all_content[-2]  # The "-2" index just deletes the second row of the data that was read in 
...
# Write filtered data into a new file. Change this when changing drives or computers
    write.csv(df,"C:\\Users\\DunwoodyE\\Documents\\Apsim73\\Regions\\M_W region\\7.3ComparisonSample\\Test_Out file_results\\output.csv", row.names = FALSE)
...
# TODO: 
# 1. Insert the code to produce the results for Simulator 7.9, 
# 2. Change names for data from Simulator version 7.3 if you like to make the graphical intent clearer
